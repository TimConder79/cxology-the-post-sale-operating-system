...
import { Activity, Zap, Calendar, TrendingUp, Users, AlertTriangle, ChevronRight, ArrowUpRight } from 'lucide-react'
...
interface SectionDef {
  types: GapType[]
  label: string
  why: string       // why this category matters for retention / growth
  Icon: LucideIcon
}
...
function GapCard({ gap }: { gap: ExecutionGap }) {
...
function Section({ def, gaps }: { def: SectionDef; gaps: ExecutionGap[] }) {
  ...
  return (
    <div className="mb-10">
      ...
      <div className="space-y-2 ml-10">
        {sorted.map(gap => <GapCard key={gap.id} gap={gap} />)}
      </div>
    </div>
  )
}
...
export function InflectionPointsView() {
  const { accounts, inflectionPoints, stakeholderMaps, contacts, playRuns } = useApp()
  ...
  const arrAtRisk    = accounts
    .filter(a => highGaps.some(g => g.accountId === a.id))
        ...
        <div className="flex items-center gap-5">
          ...
          {arrAtRisk > 0 && (
            <div className="text-right border-l border-slate-100 pl-5">
              ...
              <div className="text-[16px] font-bold text-slate-700 leading-none mt-0.5">{formatCurrency(arrAtRisk)}</div>
            </div>
          )}
        </div>
      </div>
