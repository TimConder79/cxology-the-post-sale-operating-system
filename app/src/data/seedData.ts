...
export const fiveQuestions: FiveQuestions[] = [
  // Meridian Health Systems — captured at kickoff
  {
    ...
    q1: 'Getting the EMR integration right. If patient data doesn\'t flow cleanly between systems from day one, the clinical teams will abandon the tool before it has a chance.',
    q2: 'Patient intake processing time cut by 40% within 90 days. James Whitfield measures success by whether the operations dashboard shows consistent reduction — not just a one-time improvement.',
    q3: 'We own the integration, the configuration, and the training. Meridian owns internal change management and getting the department heads to mandate usage.',
    q4: 'Two things: the EMR vendor has a history of slow API support, which is outside our control. And internally, two department heads previously championed a different tool that failed — there\'s residual skepticism we\'ll need to work through.',
    q5: 'James gets to report to the board that they solved a problem that\'s plagued them for three years. The ops team stops working late. And honestly — if Priya\'s team becomes the internal case study that drives adoption across the other two hospital systems in the network, that would be the dream.',
  },
  // Apex Logistics Group — captured at kickoff
  {
    id: 'fq-002',
    accountId: 'acc-002',
    capturedAt: '2026-01-15T00:00:00Z',
    capturedByRunId: null,
    q1: 'Eliminating the manual carrier reconciliation process. Right now that\'s where the team bleeds the most time, and it\'s where errors get introduced. If we fix that and nothing else, this was worth it.',
    q2: 'Ron defines success operationally — he wants to walk into the Q2 ops review with a before-and-after story. Linda defines it differently: her team stops doing the same manual work twice.',
    q3: 'We handle the carrier integrations and the workflow automation. Apex has to provide clean carrier contracts and designate someone to validate the reconciliation logic before we go live.',
    q4: 'The ops team is stretched thin and has a low tolerance for change right now — they\'re also mid-rollout of a new WMS. There\'s a real risk this gets deprioritized if the WMS hits problems. Carrier B also runs a non-standard EDI format that could delay the integration.',
    q5: 'If the reconciliation problem is truly solved, Ron has told us he wants to expand the platform to cover inbound freight as well. That\'s a much bigger scope — and it would mean we\'re embedded in how the whole logistics operation runs.',
  },
  // Vantage Financial — captured at kickoff, mature account
  {
    ...
    capturedByRunId: null,
    q1: 'A single, trusted source of pipeline truth across all three sales regions. Right now each region runs its own numbers, the CRO gets three different answers to the same question, and board meetings become debates about whose data is right.',
    q2: 'Claire defines success as confidence — she needs to walk into any board meeting and answer pipeline questions in real-time, without prep and without hedging. Tom defines it as the RevOps team no longer fielding ad-hoc data requests.',
    q3: 'We own the platform configuration, the data model design, and the training. Vantage owns the data cleanup in Region 2 — we can\'t do that for them — and the executive mandate to enforce consistent stage definitions going forward.',
    q4: 'Region 2\'s historical data is a mess — three years of inconsistent stage definitions. Tom knows it but it requires an exec directive to fix, and that\'s politically sensitive. There\'s also a risk that if the first version doesn\'t look polished, Claire loses confidence before we\'ve had a chance to prove value.',
    q5: 'Claire has mentioned that if this works, she wants to roll it out to their two new markets launching in H2. That would triple the scope. And if the board sees what clean pipeline data looks like, there\'s a conversation about replacing their entire BI stack — we\'d be the anchor platform.',
  },
  // ClearPath Analytics — captured at kickoff, recent account
  {
    ...
    capturedByRunId: null,
    q1: 'Proving that the platform can surface an insight the team would have genuinely missed on their own. Beth\'s team is skilled — they don\'t need another tool that tells them what they already know. It has to find something real.',
    q2: 'Beth measures success by her team\'s reaction — if they get excited about a finding, it worked. Kevin measures it by whether he can justify the platform in the Q3 budget review. Those are two different bars and we need to clear both.',
